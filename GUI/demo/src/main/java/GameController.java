...
public class GameController {
    ...
    private final boolean[][] fertilizedTiles = new boolean[SIZE][SIZE];
    private final int[][] fertilizerDaysLeft = new int[SIZE][SIZE];
    private final String[][] fertilizerName = new String[SIZE][SIZE];
    ...
    public void initialize() {
        ...
        for (int r = 0; r < SIZE; r++) {
            for (int c = 0; c < SIZE; c++) {
                ...
                fertilizedTiles[r][c] = false;
                fertilizerDaysLeft[r][c] = 0;
                fertilizerName[r][c] = "";
    ...
    private void showTileInfo(int row, int col) {
        ...
        } else if (fertilizedTiles[row][col]) {
            readyHarvestLabel.setText("Fertilizer: " + fertilizerName[row][col] + " (" + fertilizerDaysLeft[row][col] + " days left)");
        } else if (permanentFertilizer[row][col]) {
            readyHarvestLabel.setText("Fertilizer: Permanent");
        } else {
    ...
    private String getTileText(int row, int col) {
        ...
        if (fertilizedTiles[row][col] || permanentFertilizer[row][col]) {
            baseText = "(" + baseText + ")";
        }
    ...
    private void triggerMeteoriteEvent() {
        ...
        for (int[] pos : affectedTiles) {
            ...
            fertilizedTiles[row][col] = false;
            fertilizerDaysLeft[row][col] = 0;
            fertilizerName[row][col] = "";
    ...
    public boolean fertilizeSelectedTile(String name, int price, int days) {
        if (selectedRow == -1 || selectedCol == -1) return false;
        if (meteorTiles[selectedRow][selectedCol]) return false;

        if (fertilizedTiles[selectedRow][selectedCol] || permanentFertilizer[selectedRow][selectedCol]) {
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setHeaderText("Already Fertilized");
            alert.setContentText("This tile already has fertilizer.");
            alert.showAndWait();
            return false;
        }
        ...
        fertilizedTiles[selectedRow][selectedCol] = true;
        fertilizerDaysLeft[selectedRow][selectedCol] = days;
        fertilizerName[selectedRow][selectedCol] = name;
    ...
    private void handleFertilizer() {
        if (selectedRow == -1 || selectedCol == -1) {
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setHeaderText("No Tile Selected");
            alert.setContentText("Click a tile first before applying fertilizer.");
            alert.showAndWait();
            return;
        }
        ...
        if (meteorTiles[selectedRow][selectedCol]) {
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setHeaderText("Invalid Tile");
            alert.setContentText("You cannot fertilize a meteorite tile.");
            alert.showAndWait();
            return;
        }
        ...
        try {
            var resource = Main.class.getResource("/FertilizerShop.fxml");
            System.out.println(resource);

            FXMLLoader loader = new FXMLLoader(resource);
            Scene scene = new Scene(loader.load(), 1500, 850);

            FertilizerStoreController controller = loader.getController();

            Stage shopStage = new Stage();
            controller.setGameController(this);
            controller.setStage(shopStage);

            shopStage.setTitle("Fertilizer Shop");
            ...
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText("Error");
            alert.setContentText("Could not open Fertilizer Shop.");
            alert.showAndWait();
        }
    }
    ...
    private void handleNextDay() {
        ...
        for (int r = 0; r < SIZE; r++) {
            for (int c = 0; c < SIZE; c++) {
                ...
                if (fertilizedTiles[r][c]) {
                    fertilizerDaysLeft[r][c]--;
                    if (fertilizerDaysLeft[r][c] <= 0) {
                        fertilizedTiles[r][c] = false;
                        fertilizerDaysLeft[r][c] = 0;
                        fertilizerName[r][c] = "";
                    }
